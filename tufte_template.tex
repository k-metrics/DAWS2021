...
% \usepackage[noto]{zxjafont}             % Linux環境ではこちも指定可
...
\item
  ...
  \item
    必要な\(\TeX\)パッケージは自動的にインストールしてくれます
  \item
  ...
  \item
    フォーマットを変更したい場合はYAMLの\texttt{output}指定を変更してください
  \end{itemize}
\end{enumerate}
...
\tightlist
\item
  \textbf{tinytex}以外の\(\TeX / \LaTeX\)では手動でパッケージをインストールする必要があります

  \begin{itemize}
  \tightlist
  \item
    \texttt{xelatex}以外の\(\TeX / \LaTeX\)での動作は確認していません
  \item
    RStudioでの\(\LaTeX\)エンジンは必ず\texttt{xelatex}を指定します
  \end{itemize}
\item
