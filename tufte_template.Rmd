...
output:
  ...
  tufte::tufte_book:
    ...
    fig_caption: yes
    keep_tex: yes
classoption: a4paper     # 指定しないとデフォルトのUS Letterサイズ
...
* **`tidyverse`, `knitr`, `rmarkdown`, `psych`, `kableExtra`**パッケージがインストールされている（**R** 4.x推奨）
* **RStudio**（v1.4推奨）
* **Notoフォント**（Linux）、**ヒラノギフォント**（macOSは多分これかと）
...
1. 本ドキュメントをknitする
    * YAMLの`include`で指定しているファイルが必要ですので必ずテンプレートと共にコピーしてください
    * 必要な$\TeX$パッケージは自動的に**tinytex**がインストールします
    * もし、$\TeX$のメッセージが出た場合にはログを参考に必要なパッケージをインストール[^1]してください
    * 出力フォーマットを変更したい場合はYAMLの`output`指定を変更してください
...
* **tinytex**以外の$\TeX / \LaTeX$を利用する場合は手動でパッケージをインストールしてください
    ...
    * RStudioでの$\LaTeX$エンジンは必ず`xelatex`を指定してください
* 本テンプレートは必要最低限の設定になっています
    * $\TeX / \LaTeX$のデフォルト仕様として図表は自動的に再配置されます^[**`tufte`**パッケージの仕様上図表の固定はできません]
    * 余白への図などの配置はチャンクオプションで指定する必要があります^[本ドキュメントが記述仕様書になっています]
    * その他の[ドキュメントサンプル(PDF)](https://ftp.jaist.ac.jp/pub/CTAN/macros/latex/contrib/tufte-latex/sample-handout.pdf)
...
# 独自の関数定義
PDFではインタラクティブな表が使えません。また、tufteは余白が広いので通常の表出力では表示できる項目数が限られてしまいます。表現の自由度を高めるために**`kableExtra`**パッケージと**`psych`**パッケージを用いた`df_print()`関数^[詳細は`setup`チャンク内の関数が定義を参照方]を定義してあります。以下は使い方の一例です。
```{r}
mtcars[1:6, 1:6] %>% 
