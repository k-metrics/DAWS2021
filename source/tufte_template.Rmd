...
date: "`r Sys.Date()`"
abstract: "本ファイルは**`tufte`**パッケージを用いたTufte handoutフォーマットにおいて日本語が使えるような設定を施してあります。必ず同梱の`latex`フォルダと同じ階層に配置してください。"
output:
  tufte::tufte_handout:
    includes:
  ...
  tufte::tufte_book:
    ...
    fig_caption: yes
    keep_tex: yes
classoption: a4paper     # デフォルトではUS Letterサイズ
...
* **`tidyverse`**[@R-tidyverse], **`knitr`**[@knitr2014; @R-knitr], **`rmarkdown`**[@R-rmarkdown], **`psych`**[@R-psych], **`kableExtra`**[@R-kableExtra]パッケージ^[**R** 4.x推奨]
* **RStudio**^[v1.4推奨]
* **Notoフォント**（Linux）・**ヒラギノフォント**^[多分これでいけるんじゃないかと… 駄目ならNotoフォントで]（macOS）

作成手順

1. **`tufte`**[@R-tufte], **`tinytex`**[@R-tinytex; @tinytex2019]パッケージをインストールする
1. `tinytex::install_tinytex()`で**tinytex**をインストールする
1. `tinytex::tlmgr_install("haranoaji")`で原ノ味フォントをインストールする^[Windows環境のみ]
...
knitr::write_bib(c('base', 'rmarkdown', 'tidyverse', 'kableExtra', 'psych',
                   'tufte', 'knitr', 'tinytex'),
                 file = './bib/skeleton.bib')
```
